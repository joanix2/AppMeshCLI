...
class ParameterBlock(AbstractBlock):
    def __init__(self, canvas, posX, posY, text, font_size, IsInput=False, IsOutput=False, z_index=0):
        ...
        # Position des nœuds
        input_nodeX = self.posX - self.nodeRadius
        output_nodeX = self.posX + self.sizeX - self.nodeRadius
        nodeY = self.posY + self.text_height / 2 - self.nodeRadius / 2

        # Créer les nœuds avec la gestion de la visibilité
        self.input_node = Node(canvas, input_nodeX, nodeY, IsInput=True, radius=self.nodeRadius, z_index=z_index + 1, hidden=not IsInput)
        self.output_node = Node(canvas, output_nodeX, nodeY, IsInput=False, radius=self.nodeRadius, z_index=z_index + 1, hidden=not IsOutput)

        # Ajouter les nœuds au canvas
        if IsInput:
            self.canvas.add_object(self.input_node)
        if IsOutput:
            self.canvas.add_object(self.output_node)
    ...
    def draw(self):
        ...
        self.input_node.setPosition(input_nodeX, nodeY)
        self.output_node.setPosition(output_nodeX, nodeY)

        # Gérer la visibilité des nœuds
        if self.IsInput:
            self.input_node.show()
        else:
            self.input_node.hide()

        if self.IsOutput:
            self.output_node.show()
        else:
            self.output_node.hide()
...
class Node(ClickableObject):
    def __init__(self, canvas, posX, posY, IsInput, radius=5, color = "black", z_index = 0, hidden=False):
        super().__init__(canvas, posX + radius, posY + radius, radius * 2, radius * 2, color, radius, border_width=0, z_index=z_index, IsStatic=True, hidden=hidden)
        self.IsInput = IsInput
        self.target_links = []

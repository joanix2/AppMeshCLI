...
class ClickableObject:
    def __init__(self, canvas, posX, posY, sizeX, sizeY, color, radius=0, border_color="black", border_width=0, z_index=0, IsStatic=True, hidden=False):
        self.canvas = canvas
        self.posX = posX
        self.posY = posY
        self.sizeX = sizeX
    ...
    def draw(self):
        if self.hidden:
            return  # Ne pas dessiner si l'objet est caché
        ...
        if self.type == "ovale":
            ...
            if self.radius > 0:
                self.shape = create_rounded_rectangle(
                    self.canvas, self.posX, self.posY, 
                    self.posX + self.sizeX, self.posY + self.sizeY, 
                    radius=self.radius, fill=self.color, 
                    width=self.border_width
                )
            else:
    ...
    def show(self):
        ...
        if self.shape is None and not self.hidden:
            self.draw()
        elif self.hidden:
            self.hidden = False
